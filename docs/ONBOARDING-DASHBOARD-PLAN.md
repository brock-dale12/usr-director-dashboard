...
**TTV is the spine, not a side-metric.** `TTVPanel` already tracks: explicit `kickoff_date`, days-since, session-recap count vs. target (5), and a pass/fail state (manual mark > platform-synced > derived). The dashboard's onboarding health centers on TTV.

**TTV definition (locked 2026-06-30):** the 7-day window is **anchored on the manually-set `kickoff_date`** (set in the dashboard so the clock starts on the right trigger — NOT a calendar week). Pass = **≥5 `assessments_prod.assessment_reports` counted by `created_at` in `[kickoff, kickoff+7)`**, `deleted_at IS NULL`, all sources, no verification filter (the live `sync_ttv` definition). Org linkage uses the `organizations.hubspot_company_id` DB column. **Built:** `supabase/functions/sync-ttv/index.ts` writes `onboarding_ttv(status, days_to_five, recaps_in_window)`; a manual `ttv_status_override` still wins in the UI. Deploy/verify per `docs/PHASE2-SYNC-DEPLOY.md` (unrun — needs the parity check).
...
| D7 | Build on the approved `pg_cron`+Edge-Function Athena sync for health/activity/TTV; only Owen/Ruby/Casey *drafts* use a narrow vault→`suggested_emails` path | ✅ RESOLVED |
| D8 | TTV = **5 `assessments_prod.assessment_reports` in a 7-day window anchored on the manual `kickoff_date`**, counted by **`created_at`** (live `sync_ttv` def), `deleted_at IS NULL`, all sources. Org linkage via `organizations.hubspot_company_id` DB column. Built as `sync-ttv`. | ✅ RESOLVED |
| D9 | Weekly health weeks = **Sat–Fri** (Doc's convention), not the Python's Monday weeks → one-time recompute of `weekly_health_snapshots` needed when `sync-weekly-activity` ships | ✅ RESOLVED |
